...
class RMSE_log(nn.Module):
    ...
    def forward(self, valid_out, valid_gt):
        if not valid_out.shape == valid_gt.shape:
            _, _, H, W = valid_gt.shape
            valid_out = F.upsample(valid_out, size=(H, W), mode='bilinear')
        valid_out = F.relu(valid_out) + self.eps
        loss = torch.sqrt(torch.mean(torch.abs(torch.log(valid_gt) - torch.log(valid_out)) ** 2) + self.eps)
        return loss
...
class L1(nn.Module):
    ...
    def forward(self, valid_out, valid_gt):
        if not valid_out.shape == valid_gt.shape:
            _, _, H, W = valid_gt.shape
            valid_out = F.upsample(valid_out, size=(H, W), mode='bilinear')
        loss = torch.mean(torch.abs(10. * valid_gt - 10. * valid_out))
        return loss
...
class L1_log(nn.Module):
    ...
    def forward(self, valid_out, valid_gt):
        if not valid_out.shape == valid_gt.shape:
            _, _, H, W = valid_gt.shape
            valid_out = F.upsample(valid_out, size=(H, W), mode='bilinear')
        loss = torch.mean(torch.abs(torch.log(valid_gt) - torch.log(valid_out)))
        return loss
...
class RMSE(nn.Module):
    ...
    def forward(self, valid_out, valid_gt):
        if not valid_out.shape == valid_gt.shape:
            _, _, H, W = valid_gt.shape
            valid_out = F.upsample(valid_out, size=(H, W), mode='bilinear')
        loss = torch.sqrt(torch.mean(torch.abs(10. * valid_gt - 10. * valid_out) ** 2))
        return loss
...
class NormalLoss(nn.Module):
    ...
    def forward(self, valid_out, valid_gt):
        prod = (valid_out[:, :, None, :] @ valid_gt[:, :, :, None]).squeeze(-1).squeeze(-1)
        fake_norm = torch.sqrt(torch.sum(valid_out ** 2, dim=-1))
        real_norm = torch.sqrt(torch.sum(valid_gt ** 2, dim=-1))
...
class silog_loss(nn.Module):
    ...
    def forward(self, valid_out, valid_gt, mask):
        d = torch.log(valid_out[mask]) - torch.log(valid_gt[mask])
