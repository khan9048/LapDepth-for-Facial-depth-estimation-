...
parser.add_argument('--pred_path', type=str, help='path to the prediction results in jpg', default='./pre_sny/')
parser.add_argument('--gt_path', type=str, help='root path to the groundtruth data', default='./input/nyu_eval_dataset/gt/')
...
def test():
    global gt_depths, missing_ids, pred_filenames, gt_filenames
    gt_depths = []
    missing_ids = set()
    pred_filenames = []
    gt_filenames = []
    ...
    for root, dirnames, filenames in os.walk(args.pred_path):
        for pred_filename in fnmatch.filter(filenames, '*.jpg'):
            if 'Face' in pred_filename or 'gt' in pred_filename:

                dirname = root.replace(args.pred_path, '')
                # print(filenames)
                pred_filenames.append(os.path.join(dirname, pred_filename))
                #print(pred_filenames)
    ...
    ###########################################################################
    for root, dirnames, filenames in os.walk(args.gt_path):
        for gt_filename in fnmatch.filter(filenames, '*.jpg'):
            if 'Face' in gt_filename or 'gt' in gt_filename:

                dirname = root.replace(args.gt_path, '')
                # print(filenames)
                gt_filenames.append(os.path.join(dirname, gt_filename))
                #print(pred_filenames)
    num_test_samples_gt = len(gt_filenames)

    gt_depths = []

    for i in range(num_test_samples_gt):
