...
import warnings
def validate(args, val_loader, model, logger, dataset = 'NYU'):
    ##global device
...
def validate_in_test(args, val_loader, model, logger, dataset = 'NYU'):
...
def train_net(args,model, optimizer, dataset_loader,val_loader, n_epochs,logger):
    ...
    for epoch in tqdm(range(n_epochs+5)):
        ...
        for i, (rgb_data, gt_data, gt_dense) in enumerate(dataset_loader):
            ...
            RMSE_criterion = RMSE_log()
            from utils import silog_loss
            silog_criterion = silog_loss(variance_focus=args.variance_focus)

            if args.dataset == 'NYU':
                mask = valid_gt_sparse > 0.1
            else:
                mask = valid_gt_sparse > 1.0

            l_depth = l1_criterion(valid_out, valid_gt_sparse)
            l_rmse = RMSE_criterion(valid_out, valid_gt_sparse)

            # loss = scale_inv_loss + gradient_loss + l_depth + l_rmse

            loss = silog_criterion.forward(valid_out, valid_gt_sparse, mask.to(torch.bool))
            ...
            if ((i+1) % args.print_freq == 0) and (args.rank == 0):
                ...
                print("[%6d/%6d]  total: %.5f" % (
                n_iter, total_iter, loss.item()))
